...
class LinkedInToolkit(BaseLinkedInToolkit, AbstractToolkit):
    ...
    @classmethod
    def _load_token_from_file(cls, token_path: str) -> str | None:
        ...
        if expires_at and int(time.time()) >= expires_at:
            logger.warning(
                "LinkedIn token file contains expired token, skipping load"
            )
            return None

        logger.info(
            "Loaded LinkedIn credentials from token file: %s", token_path
        )
        return access_token

    def _load_credentials(self) -> str | None:
        r"""Load credentials from token file or RunContext-aware env."""
        return self._load_token_from_file(self._token_path) or env(
    ...
    def get_can_use_tools(cls, api_task_id: str) -> list[FunctionTool]:
        ...
        if cls._load_token_from_file(token_path) or env(
            "LINKEDIN_ACCESS_TOKEN"
        ):
    ...
    def is_authenticated(cls) -> bool:
        ...
        token_path = cls._build_canonical_token_path()
        return bool(
            cls._load_token_from_file(token_path)
