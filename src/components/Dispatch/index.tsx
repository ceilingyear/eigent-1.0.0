...
function buildRemoteControlTitle(spaceName?: string | null): string {
  const base = spaceName?.trim() || 'ATAI Remote Control';
    ...
    try {
      ...
      if (!bridgeReady) {
        toast.error('Remote control is still connecting.', {
          description:
            'Keep ATAI Desktop open and try again in a few seconds.',
        });
        return;
      }
      ...
      if (code === 'BRIDGE_OFFLINE') {
        toast.error('Remote control bridge is offline.', {
          description:
            'Keep ATAI Desktop open and wait for the bridge to reconnect, then try again.',
        });
      } else {
