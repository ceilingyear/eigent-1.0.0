...
function forceElectronMainCjs() {
  return {
    name: 'force-electron-main-cjs',
    config(config) {
      config.build ??= {};
      const build = config.build;
      if (build.lib && typeof build.lib === 'object') {
        build.lib = {
          ...build.lib,
          formats: ['cjs'],
          fileName: () => '[name].cjs',
  ...
  return {
    resolve: {
      alias: {
        ...
        'tiny-case': path.join(__dirname, 'src/lib/tinyCaseShim.ts'),
      },
    },
    optimizeDeps: {
      exclude: ['@stackframe/react'],
      include: ['yup', 'react-avatar-editor', 'color', 'cookie', 'qrcode'],
      force: true,
    },
    plugins: [
      react(),
