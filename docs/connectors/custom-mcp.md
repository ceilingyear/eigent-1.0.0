...
Use a custom Model Context Protocol server when you need a tool that is not included in ATAI's supported integration catalog.
...
Use an absolute path or ensure the executable is installed in the environment used by ATAI.
