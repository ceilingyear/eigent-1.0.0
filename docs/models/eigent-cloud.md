---
title: ATAI Cloud models
description: Use managed models and credits without configuring provider API keys.
icon: cloud
---

ATAI Cloud provides managed model access for users who do not want to configure provider credentials or local inference.

## When to use ATAI Cloud

Choose ATAI Cloud when you want:

- The fastest setup path
- A curated model catalog
- No provider API-key management
- Usage tracked through ATAI credits
...
Cloud usage consumes ATAI credits. Credit consumption depends on the selected model and task usage.
...
Confirm the account is signed in, the application can reach ATAI services, and the current plan includes model access.
