...
description: Review every cloud and local model provider supported by ATAI.
icon: list
---

ATAI supports multiple provider types so open-source deployments can choose models based on capability, cost, privacy, and infrastructure.
...
<Note>
Provider fields can change. Confirm required values in **Agents > Models** after updating ATAI.
</Note>
...
4. In ATAI, open **Agents > Models > Local**.
