...
description: Choose between ATAI Cloud, your own provider keys, and local inference servers.
icon: brain
---

ATAI is model-flexible by design. You can use managed ATAI Cloud models, connect provider accounts with your own keys, or run open models on local infrastructure.

At least one valid model is required before ATAI can start a task.
...
Use managed models without configuring provider credentials. This is the quickest way to evaluate ATAI and is billed through ATAI credits.
...
Connect ATAI to Ollama, vLLM, SGLang, LM Studio, or LLaMA.cpp. Local models can keep inference on infrastructure you control.
...
Removing a provider deletes its stored ATAI configuration. It does not delete the provider account or local model.
...
- [ATAI Cloud models](/models/atai-cloud)
