...
description: Start ATAI tasks from HTTP requests or supported application events.
icon: webhook
---

Webhook and app triggers start tasks when an external event occurs.

Use a webhook for a custom system. Use an application trigger when ATAI provides a guided integration for the event source.
...
ATAI generates the webhook URL after creation.
