...
Use follow-up requests to continue working in the same Project without losing earlier history. ATAI saves the original task and each follow-up as a separate **Run**.
...
ATAI adds the request to the same conversation and updates the Run selector.
...
- After a dropdown selection, ATAI keeps the selected Run while smooth scrolling reaches the requested position.
...
Suppose Run 1 asks ATAI to research a market and create a report.
