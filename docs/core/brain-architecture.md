...
description: Understand how ATAI separates clients, Brain, and runtime capabilities across desktop and web deployments.
icon: brain
---

## Overview

ATAI is evolving toward a Brain-centered architecture.

Instead of treating the desktop app as the system boundary, ATAI treats the Brain as the primary runtime. Clients such as Desktop and Web connect to the same Brain over HTTP and SSE, while runtime capabilities are determined by where the Brain is deployed.
...
The Brain is the central runtime for ATAI. It is responsible for:
...
ATAI supports multiple client shapes around the same Brain:
...
ATAI avoids that by separating:
...
That incremental approach helps ATAI evolve toward a more portable and extensible system without requiring a full rewrite.
