...
description: Navigate ATAI's Home dashboard and manage Spaces, Projects, Tasks, and Triggers.
icon: grid-2
---

The Home dashboard is the management surface for work across ATAI. Use it to find active work, review history, organize projects, and manage automations without opening each project first.
...
Spaces are top-level work areas. A Space can use an ATAI-managed scratch folder or connect directly to a local folder. Each Space contains projects, tasks, files, and triggers.
...
2. Choose **Start from scratch** for an ATAI-managed workspace, or **Use local folder** to connect existing files.
3. ATAI opens the new Space in the Workspace.
