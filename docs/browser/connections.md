...
The browser pool contains Chrome DevTools Protocol sessions that ATAI agents can use.

## Open a new browser

1. Open **Browser > Connections**.
2. Select **Open new browser**.
3. Wait for ATAI to launch the browser and add it to the pool.
...
ATAI checks `http://localhost:<port>/json/version` before adding the browser.
...
- Not already exist in the ATAI browser pool
- Be reachable from the ATAI application
...
Removing a browser disconnects it from ATAI. It does not necessarily close an external browser process.
