...
- Files stored in an ATAI scratch workspace
...
ATAI supports previews for common text, source code, Markdown, image, HTML, document, and data formats. Unsupported formats remain available in the workspace even when they cannot be previewed.
...
Blank Spaces use artifact-only mode. ATAI stores generated outputs in managed project storage instead of modifying an existing user folder.
...
ATAI opens Context and selects the file belonging to that Run.
...
Confirm that the file still exists and that ATAI has permission to access the containing folder.
