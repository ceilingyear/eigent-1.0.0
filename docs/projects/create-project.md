...
Create a Project when you want ATAI to keep related tasks, files, and follow-up requests together.
...
If no model is configured, ATAI opens the Models page instead of starting the task.
...
ATAI creates a server-backed Project in the active Space, opens the Session, and starts the first Run.

In Workforce mode, ATAI can show a task plan before execution. Review, edit, add, or remove subtasks before starting the plan.
