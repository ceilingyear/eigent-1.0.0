...
description: Connect a remote Gemini agent and delegate work from ATAI.
icon: network-wired
---

Remote sub-agents let ATAI delegate a unit of work to an externally hosted agent and poll for the result. Use them when another agent platform provides specialized capabilities or isolated execution.
...
ATAI validates the connection before saving an enabled provider.

## Enable the provider

Use the provider switch to enable or disable delegation.

When enabling an existing provider, ATAI validates required fields and the remote connection. If validation fails, the provider returns to its previous state.

## Choose timing values

### Maximum wall time

Maximum wall time limits how long ATAI waits for the remote task to finish. Set it high enough for the expected work, but low enough to prevent indefinite jobs.

### Poll interval

Poll interval controls how frequently ATAI checks the remote task. Short intervals produce faster updates but increase request volume.
